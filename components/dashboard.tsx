...
export function Dashboard({ user: initialUser, onLogout, currentView, onNavigate, onNavigateToAdmin }: DashboardProps) {
  ...
  const generateSuggestions = (currentUser: User) => {
    ...
    if (currentUser.machines.length < 5) {
      newSuggestions.push({
        ...
        description: `You have ${5 - currentUser.machines.length} portfolio slots available. Diversify to reduce risk and maximize returns.`,
    ...
    if (
      ...
      newSuggestions.push({
        ...
        description: `You have ${formatCurrency(currentUser.claimedBalance)} in realized gains ready for withdrawal.`,
        action: "Withdraw Now",
      })
    }
    ...
    if (currentUser.referrals.length < 5) {
      newSuggestions.push({
        ...
        description: `Invite ${5 - currentUser.referrals.length} more professionals to unlock exclusive investment opportunities.`,
    ...
    if (currentUser.balance < 500) {
      newSuggestions.push({
        ...
        description: "Increase your capital allocation to access premium investment strategies with higher returns.",
        action: "Invest More",
      })
    }
  ...
  const handleInvest = async () => {
    ...
    if (portfolioCount < 5 && updatedUser.balance >= (portfolioCount + 1) * 100) {
      const newPortfolio: TimeMachine = {
        id: generateId(),
        level: portfolioCount + 1,
        name: `Investment Portfolio ${portfolioCount + 1}`,
      ...
      updatedUser.machines.push(newPortfolio)
      setSuccess(`Portfolio #${newPortfolio.level} created!`)
      successConfetti()
    }
  ...
  const handleClaim = async (machineId: string) => {
    ...
    if (machine) {
      ...
      await storage.saveUser(updatedUser)
      setUser(updatedUser)
      setSuccess(`Collected ${formatCurrency(machine.rewardAmount)} in returns!`)
      successConfetti()
    }
  }
      ...
      <main className="max-w-7xl mx-auto px-4 py-12 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          ...
          <AnimatedStatCard
            ...
            description="Diversified strategies"
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold gradient-text">Investment Portfolios</h2>
                <span className="text-sm text-muted-foreground">{user.machines.length} of 5 active</span>
              </div>

              {user.machines.length === 0 ? (
                <Card className="glass glow-cyan">
                  <CardContent className="py-16 text-center">
                    <PieChart className="w-16 h-16 text-cyan-400/30 mx-auto mb-4" />
                    <p className="text-muted-foreground mb-6 text-lg">
                      No portfolios yet. Create your first investment portfolio to start earning!
                    </p>
            ...
            <Card className="glass glow-cyan">
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground">
                  <span className="text-cyan-400 font-semibold">{user.referrals.length}</span> professionals referred
                </p>
              </CardContent>
            </Card>
          </div>
